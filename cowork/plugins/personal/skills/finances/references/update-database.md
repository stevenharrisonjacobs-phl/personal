...
## 0. The Cowork ceiling — diagnose here, fix in Claude Code

**If you are running through the door (no repo, no `bq`), you can do steps 1–3
and nothing else.** Every fix in step 4 — rules, overrides, `deploy.sh` — is a
write, and the door has no write tools by design. A classification change made
from a phone reshapes the books with no review step.

So from a Cowork seat: diagnose fully, name the exact fix, and stop. Say plainly
that it needs a Claude Code session in the repo. Do not improvise a workaround.

## 1. Check feed freshness (always do this first)

```
feed_health()                                     # through the door
./scripts/query.sh queries/source-freshness.sql   # locally
```
...
### 3a. The silent-vendor check — do this every time

A stale *feed* is only half the failure mode. The other half is a vendor that
was paid reliably and then **stopped**, while the feed stayed healthy. That is
how three mortgage payments went unnoticed for a quarter: nothing was broken,
something was simply absent, and absence does not raise an alert.

For any vendor billing in **≥6 of the last 12 months** at a consistent amount,
flag it when it goes quiet past its usual window. Report the vendor, its usual
amount and cadence, and how long it has been silent.

Absence is the hardest thing for a query to notice and the most expensive thing
to miss — so look for it deliberately rather than waiting for it to surface.
