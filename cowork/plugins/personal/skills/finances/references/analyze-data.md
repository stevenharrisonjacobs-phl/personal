...
expenses are negative). Read `context/definitions.md` before any metric claim.

**Through the door (Cowork, or anywhere the connector is present):**

1. `list_saved_queries()` — if one fits, `saved_query(name)` and stop. It already
   encodes the sign conventions and exclusions.
2. Otherwise `list_finance_sources()` → `describe_finance_source(source)` to get
   the **live** schema — never guess column names — then `run_finance_query(sql)`.
3. Capped at 500 rows and 1 GiB. If you hit the cap, aggregate or narrow; do not
   paginate your way around it.

**Locally (Claude Code against the repo):**
...
Never mix the two transports in one session.
