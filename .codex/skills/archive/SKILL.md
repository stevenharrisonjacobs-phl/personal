---
name: archive
description: Safely archive the current Conductor workspace — commit & push all work, write a durable handoff/changelog so someone can understand the work cold, and capture the Codex session transcripts for later debugging. Use when the user says "archive this workspace", "/archive", "wrap up and archive", or is about to delete/archive a Conductor worktree.
metadata:
  short-description: Commit, push, hand off, and capture transcripts before a Conductor worktree is deleted
---
...
2. The **Codex session transcripts** at `$CODEX_HOME/sessions/YYYY/MM/DD/rollout-*.jsonl`
   (`CODEX_HOME` defaults to `~/.codex`) — but Codex files them by *date* and only
   records the workspace path inside each file (the `cwd` in its `session_meta`
   line), so they are easy to lose track of once the worktree is gone.
...
# huge session history? limit the scan:  SINCE_DAYS=30 bash "<skill-base-dir>/scripts/collect-transcripts.sh"
```

- Scans `$CODEX_HOME/sessions/**/rollout-*.jsonl` and keeps every rollout whose
  `session_meta.payload.cwd` equals this workspace's path, copying them to
  `<ARCHIVE_ROOT>/<workspace>-<date>/transcripts/` (never-reused, date-stamped).
- Emits a `## Session transcripts (Codex)` manifest (session IDs, message counts,
  timestamps, branch-at-capture). **Keep that manifest** — you'll paste it into the
  handoff in step 5.
- Requires `jq`. If the helper prints `NO_TRANSCRIPTS`, note that in the handoff and
  continue (the workspace may never have run Codex, or ran under a different path).
...
- Stage and commit. Follow repo commit conventions (see root `AGENTS.md` / `CLAUDE.md`),
  and end the message with the required `Co-Authored-By` trailer if the repo uses one.
...
`## Session transcripts (Codex)` manifest from step 2** to it.
...
- **The raw transcripts** only exist on the Mac that ran the session. The helper
  ...
  - If the two Macs use **different Apple IDs**, or you want access-controlled /
    ...
    ```
    Record the `gs://` path in the handoff so it's findable from any machine.
